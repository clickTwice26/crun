...
def cout(message: str, type: str = "info"):
    ...
    if type == "info":
        ...
        colorType = Back.CYAN
    print(
        Fore.CYAN + prefix + Style.RESET_ALL + colorType + message + Style.RESET_ALL,
        end="\n",
    )
...
def clear():
    ...
    sb.run(cmd, shell=True, check=False)  # migrated to sb from os.system


def checkFileName(fileName: str) -> bool:
    file_name_as_list: list[str] = fileName.split(" ")
    return len(file_name_as_list) == 1
...
class Capsule:
    def __init__(self, location, file_name, output_name, extra_command=None) -> None:
        ...
        self.sourceFileLocation = f"{self.location}/{self.file_name}"
        self.outputFileLocation = f"{self.location}/{self.output_name}"
        self.extra_command = extra_command
        debugMsg(f"extra-command : {self.extra_command}")
        ...
        for i in os.listdir(self.location):
            if i.endswith(".out"):
                try:
                    ...
                    cout(f"Failed to remove {self.location}/{i} because of :{error}")
        if counter < 1:
            cout("No .out file is there")
                ...
                try:
                    ...
                    if operation == "q":
                        ...
                        try:
                            ...
                            shutil.copy(
                                f"{self.sourceFileLocation}",
                                f"{os.getcwd()}/saves/{save_file_name}",
                            )
                        except Exception as error:
                            # debugMsg(error, "error")
                            cout(
                                f"SAVES directory not found. Creating one at {os.getcwd()}/saves",
                                "warning",
                            )
                            os.mkdir(f"{os.getcwd()}/saves/")
                            shutil.copy(
                                self.sourceFileLocation,
                                f"{os.getcwd()}/saves/{save_file_name}",
                            )
                        try:
                        ...
                        while True:
                            ...
                            try:
                                if message != "":
                                    print("=>" + message)
                                cout(
                                    f"Refresh[Ctrl+C]\tQuit[q/Q]\tCounter[{counter}/{max_counter}]\nSelect File:",
                                    "input",
                                )
                                selection = input()
                                if selection.lower() == "q":
                                    ...
                                    message = (
                                        f"Selected File: {all_files[int(selection)]}"
                                    )
...
if __name__ == "__main__":
    ...
    debugMsg(f"options : {','.join(options)}")
    # param = sys.argv
    if len(options) > 0:
    ...
    try:
        file_output_name = (
            ...
            else f"{file_name}_{randint(1, 1000)}.out"
        )
        file_output_name = (
            file_output_name
            if file_output_name.endswith(".out")
            else file_output_name + ".out"
        )
    except IndexError:
        cout(
            f"[+] Default file output name selected: {file_name}_{randint(1, 1000)}",
            "info",
        )
